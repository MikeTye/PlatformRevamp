...
export function AppLayout({ children }: AppLayoutProps) {
    ...
    const handleProjectWizardClose = (
        result?: { completed: false } | { completed: true; companyId?: string | null; projectId?: string | null }
    ) => {
        console.log('ProjectWizard close result:', result);
        setIsProjectWizardOpen(false);

        if (result?.completed && result.projectId) {
            console.log('Navigating to:', `/my-projects/${result.projectId}`);
            navigate(`/my-projects/${result.projectId}`);
        }
    };

    const handleCompanyWizardClose = (
        result?:
            | { completed: false }
        ...
        if (result?.completed && result.companyId) {
            navigate(`/my-company/${result.companyId}`);
        }
    };

    const handleUpdateWizardClose = (result?: UpdateWizardCloseResult) => {
        ...
        if (result?.completed && result.projectId) {
            ...
            if (result.updateId) {
                params.set('highlightUpdate', result.updateId);
            }

            navigate(`/my-projects/${result.projectId}?${params.toString()}`);
        }
    };
    ...
    return (
        <Box display="flex" minHeight="100vh" bgcolor="grey.100">
            ...
            <UpdateWizard
                ...
                onClose={handleUpdateWizardClose}
            />
