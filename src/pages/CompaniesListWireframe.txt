...
export function CompaniesListWireframe() {
    ...
    const handleServiceCategoryFilterChange = (event: SelectChangeEvent<string[]>) => {
        const value = event.target.value;
        setServiceCategoryFilter(typeof value === 'string' ? value.split(',') : value);
    };

    const handleCountryFilterChange = (event: SelectChangeEvent<string[]>) => {
        const value = event.target.value;
        setCountryFilter(typeof value === 'string' ? value.split(',') : value);
    };
